...
const fetchTableData = cache(async (route: string, track: string, gene: string) => {
    // FIXME: temporary while collection metadata queried out of FILER only
    ///filer/qtl/NGFGXQTL0000101/?loc=ABCA5&page=1
    const requestUrl = `/api/${route}/qtl/${track}/?loc=${gene}&view=table`;
    let data = null;
    try {
        ...
        if (response.ok) {
            ...
            throw new Error(`Error fetching track data from ${track} for ${gene} from route ${route}`);
        }
    } catch (error) {
        console.error(error);
    } finally {
...
function GeneDataTable({ track, gene }: Props) {
    ...
    useEffect(() => {
        try {
            ...
            fetchTableData(route, track, gene).then((result) => setData(result));
        } catch (err) {
